...
const content = \import { Star, Quote } from "lucide-react";
  ...
  return (
    <section className="py-24 bg-[#F9FAFB] relative overflow-hidden">
      ...
      <div className="testimonial-wrapper w-full relative pt-4 pb-12 overflow-hidden mx-auto max-w-[1600px]">
        {/* Edge Fade Masks */}
        <div className="absolute top-0 left-0 w-24 md:w-40 h-full bg-gradient-to-r from-[#F9FAFB] to-transparent z-10 pointer-events-none" />
        <div className="absolute top-0 right-0 w-24 md:w-40 h-full bg-gradient-to-l from-[#F9FAFB] to-transparent z-10 pointer-events-none" />

        <div className="testimonial-track px-6">
          {allTestimonials.map((testimonial, idx) => (
            <div key={\\-\\} className="testimonial-card group">
              <div className="testimonial-splash" />
              
              <Quote className="absolute top-6 right-6 w-12 h-12 text-gray-50 group-hover:-translate-y-1 group-hover:scale-105 group-hover:text-[#FF8C00]/10 transition-all duration-500 z-0" strokeWidth={1} />
              ...
              <div className="relative z-10 h-full flex flex-col">
                ...
                <p className="text-base leading-relaxed mb-10 italic flex-grow" style={{ color: "#4A5A6A" }}> 
                  "{testimonial.text}"
                </p>

                <div className="mt-auto border-t border-gray-100 pt-4">
                  <p className="font-heading font-semibold text-[17px] mb-0.5" style={{ color: "#0F2A44" }}> 
                    {testimonial.name}
                  </p>
                  <p className="text-[13px] font-medium" style={{ color: "#8A9AA8" }}>
...
fs.writeFileSync('src/components/Testimonials.jsx', content, 'utf8');
